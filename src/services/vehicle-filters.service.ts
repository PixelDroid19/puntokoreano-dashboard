...
class VehicleFiltersService {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          error.response?.data?.message ||
            "Error al obtener los filtros de vehículos"
        );
      }
      throw error;
    }
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        return {
          ...
          status: "failed",
          message:
            error.response?.data?.message || "Error al sincronizar los filtros",
        };
      }
      throw error;
    }
  ...
  static convertToTreeView(filter: Filter): FilterTreeNode[] {
    ...
    Object.keys(filter.families).forEach((familyKey) => {
      ...
      filter.families[familyKey].forEach((model) => {
        ...
        if (
          filter.transmissions[familyKey] &&
          filter.transmissions[familyKey][model.value]
        ) {
          filter.transmissions[familyKey][model.value].forEach(
            (transmission) => {
  ...
  static saveFilterSearch(
    name: string,
    criteria: SavedFilterSearch["criteria"]
  ): SavedFilterSearch {
